...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    try {
      const requestsCollection = adminDb.collection("access_requests");
      const pendingQuery = requestsCollection.where("status", "==", "pending");
      const requestsSnapshot = await pendingQuery.get();
      
      const requests = requestsSnapshot.docs.map(doc => ({
        id: doc.id,
        email: doc.data().email || doc.id, // Fallback to doc.id if email field is missing
      ...
      return res.status(200).json({ requests: sortedRequests });
    } catch (error) {
